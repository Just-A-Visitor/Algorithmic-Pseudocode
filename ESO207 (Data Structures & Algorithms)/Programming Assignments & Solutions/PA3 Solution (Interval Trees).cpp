...
Interval makeInterval(int low, int high)
{
    Interval interval;
    interval.low = low;
    interval.high = high;
...
void fixupForMax(Node *node)
{
    ...
    int oldMax = node->max;
    int newMax = maximum( (node->interval).high, getMax(node->left), getMax(node->right));
...
/* Searches if the key of the interval is present in the set or not */
/* If present, returns the node with key */
Node* searchKeyOfInterval(Node *root, Interval interval)
{
    /* Catch:- Even if the keys match, we need to check the high values, whether they match or not */
    if(root == nullptr || interval.low == root->key)
    {
...
Node* getOverallMinimum(Node *a, Node *b, Node *c)
{
    ...
    int val_a = getHighValue(a);
    int val_b = getHighValue(b);
    int val_c = getHighValue(c);
...
/* Searches if the key of the interval is present in the set or not */
/* If present, returns the node with key */
Node* fake_searchKeyOfInterval(Node *root, Interval interval)
{
    /* Catch:- Even if the keys match, we need to check the high values, whether they match or not */
    if(root == nullptr || interval.high == root->key)
    {
    ...
    if(interval.high < root->key) fake_searchKeyOfInterval(root->left, interval);
    else fake_searchKeyOfInterval(root->right, interval);
}
...
void answerQuery(struct Token token[], int length)
{
    Node *root = nullptr;
...
void scanAndAnswerQueries()
{
    ...
    while (operation<len)
    {
        ...
        else if(temp == 'm')
        {
            ...
            if(temp == 'n')
            {
                printInterval (lowFieldMinInterval(root));
                operation++;
            }
            ...
            {
                printInterval (highFieldMaxInterval(root));
                operation++;
            }
        ...
        else if(temp == 'l')
        {
            ...
            printInterval( lowFieldSuccessor(searchKeyOfInterval(root,interval)) );
        ...
        else if(temp == 'h')
        {
            ...
            printInterval( fake_highFieldSuccessor(fake_searchKeyOfInterval(fake_root,interval)) );
