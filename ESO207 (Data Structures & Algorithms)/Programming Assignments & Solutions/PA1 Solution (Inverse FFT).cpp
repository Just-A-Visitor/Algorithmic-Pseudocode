...
#include <iomanip>
using namespace std;
const double PI =M_PI; //Remember the = and ; sign
...
/* The function also asks whether we need to divide all entries by the number flag */ 
void print_complex_vec(vector<complex_num> temp, int length, int flag_divide_by_length)
{
    ...
    for(int i=0; i<length; i++)
    {
        ...
        if(fabs(y)<0.00001) y=0.00000;
        printf("%0.5lf ",y);  //Precision is 5
    }
    printf("\n");
}
/* Note :-- The function has been modified to divide the result by n(if necessary) and then print it*/
...
vector<complex_num> Inverse_FFT(vector<complex_num> Vec, int n)
{
    ...
    {
        /*In the initial iteration, remember, w=w_0 */
        Y[k]=Y_0[k]+w*Y_1[k];    //Remember to divide by n outside
        Y[k+n/2]=Y_0[k]-w*Y_1[k];  //Rememer to divide by n outside
